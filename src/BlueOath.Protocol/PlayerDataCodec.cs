...
/// <summary>THeroGrid.CombinationInfo。</summary>
public sealed record HeroCombinationInfo(int ComLv = 0, int ComGrade = 0, uint Combine = 0, uint BeCombined = 0);
...
public sealed record HeroGrid(uint HeroId = 0, int TemplateId = 0, int Lvl = 0, int Fashioning = 0,
    ...
    HeroCombinationInfo? CombinationInfo = null);
...
var reader = new GameLoginCodec.ProtoReader(payload);
    ...
    public static byte[] Encode(HeroGrid value)
    {
        ...
        var combination = value.CombinationInfo ?? new HeroCombinationInfo();
        using (var combinationOutput = new MemoryStream())
        {
            WriteVarintField(combinationOutput, 1, unchecked((ulong)combination.ComLv));
            WriteVarintField(combinationOutput, 2, unchecked((ulong)combination.ComGrade));
            WriteVarintField(combinationOutput, 3, combination.Combine);
            WriteVarintField(combinationOutput, 4, combination.BeCombined);
            WriteMessage(output, 27, combinationOutput.ToArray());
        }
