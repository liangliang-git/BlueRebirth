...
mod route_guard_tests {
    ...
    fn login_bootstrap_prioritizes_last_battle_copy_type() {
        ...
        account.guide.settings.insert(
            ...
            r#"{"NewCopyButtomIndex":1,"OtherSetting":7}"#.to_owned(),
        );
        ...
        let fields =
            blueoath_protocol::decode_varint_fields(&first_copy.payload.clone().into_bytes())
                .unwrap();
        assert_eq!(fields.get(&3), Some(&vec![2]));
        let prefs = pre
            .into_iter()
            .find(|response| response.method == "prefs.UpdatePrefsInfo")
            .expect("navigation preference bootstrap response");
        let prefs = decode_string_field(&prefs.payload.into_bytes(), 1).expect("prefs json");
        let prefs = serde_json::from_str::<serde_json::Value>(&prefs).expect("valid prefs json");
        assert_eq!(prefs["NewCopyButtomIndex"], 2);
        assert_eq!(prefs["OtherSetting"], 7);
    }
