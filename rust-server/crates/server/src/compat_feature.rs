...
pub(crate) mod legacy_test_handler {
    ...
    use super::super::battle_state::{battle_copy_passed, record_battle_pass};
    use super::super::catalog::GameLoginCatalogs;
    use super::*;

    pub(crate) fn handle<'state, 'account, 'scratch>(
        ...
        match method {
            ...
            "illustrate.VowDecTime" => {
                ...
                if items.is_empty() {
                    *handler_error = Some(GameError::Internal(
                    ...
                    if items.iter().any(|(item_id, count)| {
            ...
            "hero.Marry" => {
                ...
                if json_i64(hero, "marryTime").unwrap_or_default() != 0 {
                    *handler_error =
                        Some(GameError::Internal("hero is already married".to_owned()));
                    return Some(Vec::new());
                }
                ...
                let total_cost = hero_ids
                    ...
                    .filter_map(|hero_id| {
                        ...
                        let fixed_money = SHIP_STAT_CATALOG
                            ...
                            .unwrap_or_default();
                        Some(
                            (fixed_money.max(0) * (HP_COEFFICIENT - cur_hp.max(0))
                                + HP_COEFFICIENT
                                - 1)
                                / HP_COEFFICIENT,
                        )
            ...
            "task.GetPtReward" => {
                ...
                let Some(configured_reward_id) = catalog.teaching_rewards_by_id.get(&reward_id)
                else {
                    *handler_error = Some(GameError::Internal(
                ...
                let Some(tasks) = tasks else {
                    *handler_error =
                        Some(GameError::Internal("task state is unavailable".to_owned()));
                    return Some(Vec::new());
                };
                let ids = tasks
    ...
    pub(crate) fn pass_mini_game(
        ...
        if let Some(catalog) = battle_catalog {
            if !catalog.copies.contains_key(&copy_id)
                && !chapter_catalog
                    .is_some_and(|chapters| chapters.mini_game_ids.contains(&copy_id))
            {
                *handler_error = Some(GameError::Internal(
    ...
    fn modify_vow_hero_list(
        ...
        let template_ids = account
            ...
            .flatten()
            .filter(|hero| {
                json_u64(hero, "heroId").is_some_and(|id| hero_ids.contains(&(id as i32)))
            })
    ...
    fn claim_copy_star_reward(
        ...
        for index in indexes {
            ...
            let Some(required_stars) = chapter_rewards.star_conditions.get(position).copied()
            else {
                *handler_error = Some(GameError::Internal(
                    "copy star reward index is invalid".to_owned(),
                ));
