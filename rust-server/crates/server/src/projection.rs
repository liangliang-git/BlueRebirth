    ...
    while index < payload.len() {
        ...
        match (field, wire) {
            (1, 2) => {
                ...
                fleet.tactics.push(decode_fleet_tactic(body));
                index = end;
            }
            (2, 0) => {
                ...
                fleet.max_power = i32::try_from(value).unwrap_or_default();
                index = next;
            }
            (3, 0) => {
                let Ok((value, next)) = read_varint(payload, index) else {
                ...
                fleet.min_power = i32::try_from(value).unwrap_or_default();
                index = next;
            }
            _ => {
...
pub(super) fn decode_fleet_tactic(payload: &[u8]) -> FleetTactic {
    let mut index = 0;
    let mut tactic = FleetTactic {
        formation_id: 2,
        tactic_type: 1,
        ..FleetTactic::default()
    };
    while index < payload.len() {
        ...
        match (field, wire) {
            (1, 2) => {
                let Some((value, next)) = read_string_slice(payload, index) else {
                    break;
                };
                tactic.tactic_name = String::from_utf8_lossy(value).into_owned();
                index = next;
            }
            (2, 0) => {
                let Ok((value, next)) = read_varint(payload, index) else {
            ...
            (3, 0) => {
                ...
                tactic.mode_id = i32::try_from(value).unwrap_or_default();
                index = next;
            }
            (4, 0) => {
                let Ok((value, next)) = read_varint(payload, index) else {
                ...
                tactic.strategy_id = i32::try_from(value).unwrap_or_default();
                index = next;
            }
            (5, 0) => {
                let Ok((value, next)) = read_varint(payload, index) else {
                ...
                tactic.formation_id = i32::try_from(value).unwrap_or_default();
                index = next;
            }
            (6, 0) => {
                let Ok((value, next)) = read_varint(payload, index) else {
                ...
                tactic.tactic_type = i32::try_from(value).unwrap_or_default();
                index = next;
            }
            (7, 0) => {
                let Ok((value, next)) = read_varint(payload, index) else {
