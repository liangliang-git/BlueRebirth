...
当前项目是本地单机客户端服务端，账号数据主要由单个进程访问，SQLite 最合适：

- 零外部服务依赖，客户端开箱即用。
- 单文件部署、备份和重置简单。
- 账号级事务足够覆盖战斗结算、奖励、资源、任务更新。
- 当前 Rust Storage 已使用 `rusqlite`，迁移成本最低。

数据库改造阶段继续使用 SQLite，但改成强类型领域模型对应的规范化表，不再保存整份 `account_json`。

### 后续扩展：PostgreSQL

当出现以下需求时切换 PostgreSQL：

- 多个游戏服务实例同时写同一数据库。
- 多个远程玩家并发在线。
- 独立 GM、后台、排行榜、统计服务直接访问数据库。
- 需要高可用、远程备份、连接池、细粒度锁和在线运维。
- 账号数量和写入并发明显超过单机 SQLite 的实际承载范围。

从第一阶段就隔离 `AccountRepository`，让领域层不依赖 `rusqlite`；未来增加 `blueoath-storage-postgres`，不改 Handler 和业务规则。

### 不选 MongoDB 作为主库

本项目核心数据具备强关系和强事务特征：

- 账号、舰船、装备、装备槽、舰队存在外键关系。
- 战斗结算同时修改多个资源、进度、奖励和记录。
- 领奖、装备绑定、好友关系需要唯一约束和幂等。
- 任务、每日副本、建造队列需要范围查询和组合索引。

MongoDB 能保存嵌套文档，但会把关系约束、跨模块事务和唯一性校验推回应用层，不能解决当前混乱，反而增加一致性风险。活动日志或非结构化分析数据可单独考虑 MongoDB，不作为账号主库。

### 统一抽象

```text
domain/game  →  AccountRepository trait  →  storage-sqlite
                                      └──→  storage-postgres（未来）
```

第一版只实现 SQLite；Repository、事务边界、错误类型和查询职责保持数据库无关。这样可先完成领域重构，再按真实并发数据决定是否迁移 PostgreSQL。
